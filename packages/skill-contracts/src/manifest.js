...
const normalizeSkillPath = (value) => value.replaceAll('\\', '/');
const globMetacharacterPattern = /[*?\[\]{}]/;
const isWindowsDriveRelativePath = (value) => /^[A-Za-z]:(?![\\/])/.test(value);
const isAbsolutePath = (value) => {
    const normalized = normalizeSkillPath(value);
    return path.posix.isAbsolute(normalized) || path.win32.isAbsolute(value);
};
const hasParentDirectoryTraversal = (value) => normalizeSkillPath(value)
    .split('/')
    .some((segment) => segment === '..');
const escapesSkillRoot = (value) => {
...
const isRelativeWithinSkillRoot = (value) => {
...
const createRelativeSkillPathSchema = (fieldName) => nonEmptyString.refine(isRelativeWithinSkillRoot, `${fieldName} must be a relative path within the skill root`);
const createEntryPathSchema = () => createRelativeSkillPathSchema('entry').refine((value) => !globMetacharacterPattern.test(value), 'entry must be a relative file path within the skill root and not a glob pattern');
